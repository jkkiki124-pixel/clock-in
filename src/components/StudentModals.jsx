...
export function StudentModal({ student, onClose, onUpdate, onDelete, togglePayment }) {
  ...
  const isPaid       = monthPayment?.paid ?? false;
    ...
    <BottomSheet onClose={onClose}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 20 }}>
        <div style={{
          ...
          fontWeight: 700, fontSize: 22, color: C.accent, flexShrink: 0,
        }}>
          {student.name[0]}
        </div>
        <div>
          <div style={{ fontWeight: 700, fontSize: 20 }}>{student.name}</div>
          <div style={{ fontSize: 13, color: C.inkMuted }}>{student.grade} · 등록일 {student.registeredAt}</div>
        </div>
        <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
          <button
            onClick={() => { setEditing(!editing); setForm(student); }}
            style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 8, padding: "6px 12px", fontSize: 13, color: C.inkMuted }}
          >
            {editing ? "취소" : "✏️ 수정"}
          </button>
        </div>
          ...
          <div style={{ marginTop: 12 }}>
            {!confirmDelete ? (
              <button
                ...
                style={{ width: "100%", padding: "11px", borderRadius: 10, border: `1px solid ${C.accent}`, background: "none", color: C.accent, fontWeight: 600, fontSize: 14 }}
              >
                🗑️ 학생 삭제
              </button>
            ) : (
              <div style={{ background: C.accentLight, borderRadius: 10, padding: "14px 16px", textAlign: "center" }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={() => setConfirmDelete(false)} style={{ flex: 1, padding: "9px", borderRadius: 8, border: `1px solid ${C.border}`, background: C.surface, fontWeight: 600, fontSize: 14 }}>취소</button>
                  <button onClick={handleDelete} style={{ flex: 1, padding: "9px", borderRadius: 8, border: "none", background: C.accent, color: "#fff", fontWeight: 700, fontSize: 14 }}>삭제</button>
                </div>
              </div>
            )}
          </div>
